...
public class OKhttpHelper {
    ...
    public static final String GET_MYROOMID = "http://123.56.201.72/sxb/index.php?svc=user_av_room&cmd=get";

    // 开启直播
    // public static final String NEW_ROOM_INFO = "http://182.254.234.225/sxb/index.php?svc=live&cmd=start";
    public static final String NEW_ROOM_INFO = "http://123.56.201.72/sxb/index.php?svc=live&cmd=start";


    // 关闭直播
    // public static final String STOP_ROOM = "http://182.254.234.225/sxb/index.php?svc=live&cmd=end";
    public static final String STOP_ROOM = "http://123.56.201.72/sxb/index.php?svc=live&cmd=end";

    // 直播列表
    // public static final String GET_LIVELIST = "http://182.254.234.225/sxb/index.php?svc=live&cmd=list";
    public static final String GET_LIVELIST = "http://123.56.201.72/sxb/index.php?svc=live&cmd=list";

    // 心跳
    // public static final String SEND_HEARTBEAT = "http://182.254.234.225/sxb/index.php?svc=live&cmd=host_heartbeat";
    public static final String SEND_HEARTBEAT = "http://123.56.201.72/sxb/index.php?svc=live&cmd=host_heartbeat";

    // 直播封面
    // public static final String GET_COS_SIG = "http://182.254.234.225/sxb/index.php?svc=cos&cmd=get_sign";
    public static final String GET_COS_SIG = "http://123.56.201.72/sxb/index.php?svc=cos&cmd=get_sign";

    // 登陆
    public static final String LOGIN_SERVER = "http://123.56.201.72/sxb/index.php?svc=user&cmd=login";
     ...
     * Login
     */
    public ArrayList<String> loginLogic(String userName, String password) {
        try {
            JSONObject loginJson = new JSONObject();
            loginJson.put("username", userName);
            loginJson.put("password", password);
            String json = loginJson.toString();
            String res = post(LOGIN_SERVER, json);
            SxbLog.i(TAG, "loginLogic: " + res);
            JSONTokener jsonParser = new JSONTokener(res);
            JSONObject response = (JSONObject) jsonParser.nextValue();
            ...
            if (code == 0) {
                ...
                ArrayList<String> result = new Gson().fromJson(sign.toString(), listType);
                return result;
            }
    ...
    public String getCosSig() {
        try {
            JSONObject req = new JSONObject();
            req.put("bucket", "portrait");

            String response = OKhttpHelper.getInstance().post(GET_COS_SIG, req.toString());
